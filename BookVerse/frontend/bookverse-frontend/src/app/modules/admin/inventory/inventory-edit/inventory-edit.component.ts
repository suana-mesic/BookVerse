...
export class InventoryEditComponent extends BaseComponent implements OnInit {
  ...
  //combineLatest comes from RXJS
  //combineLatest listens for changes in 2 sources at the same time: books autocomplete and stores form field
  //whenever ANY of THESE changes, it automatically FILTERS the book list again
  private setFilteredBooksOptions(): void {
    const storeChanges = this.form
      .get('storeId')!
      .valueChanges.pipe(startWith(this.form.get('storeId')!.value as number));

    this.filteredBooksOptions = combineLatest([
      this.booksAutocompleteInput.valueChanges.pipe(startWith('')),
      storeChanges,
    ]).pipe(
      map(([value, storeId]) => {
        const filterText =
          typeof value === 'number'
            ? (this.books.find((b) => b.id === value)?.title ?? '').toLowerCase()
            : (value ?? '').toString().toLowerCase();

        if (typeof value === 'string' && value !== '') {
          const exact = this.books.find((b) => b.title.toLowerCase() === value.toLowerCase());
          if (exact) {
            if (this.form?.get('bookId')?.value !== exact.id)
              this.form?.get('bookId')?.setValue(exact.id, { emitEvent: false });
          } else if (this.form?.get('bookId')?.value) {
            this.form?.get('bookId')?.setValue(null, { emitEvent: false });
          }
        }

        return this.books.filter((book) => {
          if (!book.title.toLowerCase().includes(filterText)) return false;
          if (!storeId) return true;
          if (storeId === this.storeId && book.id === this.bookId) return true;
          const available = this.storeBookPairs![storeId];
          return !!available && !!available[book.id];
        });
      }),
    );
  }
  ...
  private loadForm() {
    this.inventoryApi.getById(this.storeId, this.bookId).subscribe({
      next: (response) => {
        ...
        this.createForm();
        this.loadStores();
        //waits for the HTTP request to GET books and pairs and then calls setFilteredBooksOptions()
        //forkJoin waits for both books and pairs to come from backend and then calls setFilteredBooksOptions()

        forkJoin({
          books: this.booksApi.listBooksForAutocomplete(),
          pairs: this.inventoryApi.getStoreBookPairs(),
        }).subscribe({
          next: ({ books, pairs }) => {
            this.books = books;
            this.storeBookPairs = pairs;
  ...
  onSubmit() {
    ...
    const comamnd = {
      ...
      reorderTreshold: +raw.reorderTreshold,
    };
