 ...
 */
const SILENT_ERROR_URL_PATTERNS = ['countriesnow.space', 'opendatasoft.com', 'restcountries.com'];

export const errorLoggingInterceptor: HttpInterceptorFn = (req, next) => {
  return next(req).pipe(
    catchError((error: HttpErrorResponse) => {
      const isSilent = SILENT_ERROR_URL_PATTERNS.some((p) => req.url.includes(p));
      if (isSilent) {
