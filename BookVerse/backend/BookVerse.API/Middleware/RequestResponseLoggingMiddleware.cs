    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        finally
        {
            ...
            if (elapsed > SlowRequestThresholdMs)
            {
                ...
                // >>> Bugfix 27.10.2025: safe disk write (does not break the response if the folder does not exist)
                try
                {
                    var logDir = Path.Combine(AppContext.BaseDirectory, "Logs");
                    Directory.CreateDirectory(logDir);
                    var logPath = Path.Combine(logDir, "slow-requests.log");
                    await File.AppendAllTextAsync(logPath, $"{DateTime.UtcNow:u} | {request.Path} | {elapsed} ms{Environment.NewLine}", context.RequestAborted);
                }
                catch (Exception ex)
                {
            ...
            // >>> Bugfix 27.10.2025: CRITICAL: restore the original stream and copy the body back so the json error message is visible
