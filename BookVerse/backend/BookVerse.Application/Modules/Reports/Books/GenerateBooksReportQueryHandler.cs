        ...
        : IRequestHandler<GenerateBooksReportQuery, byte[]>
    {
        ...
        public async Task<byte[]> Handle(GenerateBooksReportQuery request, CancellationToken ct)
        {
            ...
            var items = await query
                ...
                {
                    x.OrderId,
                    OrderDate = x.Order.CreatedAtUtc,
                    x.Quantity,
                    x.PriceAtTime,
                    Total = x.Quantity * x.PriceAtTime
                }).ToListAsync(ct);
            ...
            var totalRevenue = items.Sum(x => x.Total);
                ...
                container.Page(page =>
                {
                    ...
                    page.Content().Column(col =>
                    {
                        col.Item().PaddingVertical(10).Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                foreach (var columnName in new[] { translatePdf(lang,"orderId"), translatePdf(lang,"date"), translatePdf(lang,"qty"), translatePdf(lang,"price"), translatePdf(lang,"total") })
                                    header.Cell().Background(Colors.Green.Medium).Padding(5)
                                        .Text(columnName).FontColor(Colors.White).SemiBold();
                            });

                            foreach (var item in items)
                            {
                                table.Cell().Padding(5).Text(item.OrderId.ToString());
                                table.Cell().Padding(5).Text(item.OrderDate.ToString(dateFormat, culture));
                                table.Cell().Padding(5).Text(item.Quantity.ToString());
                                table.Cell().Padding(5).Text(item.PriceAtTime.ToString("N2", culture));
                                table.Cell().Padding(5).Text(item.Total.ToString("N2", culture));
                            }
                        });

                        col.Item().PaddingTop(20).BorderTop(1).PaddingTop(10).Row(row =>
                        {
                            row.RelativeItem().Text($"{translatePdf(lang, "totalQty")}: {totalQuantity} {translatePdf(lang, "unit")}").SemiBold();
