...
public class BookFormatsEdgeCasesIntegrationTests
{
    ...
    {
        var response = await _client.GetAsync("/BookFormats?Paging.PageSize=1");
    ...
    {
        var response = await _client.GetAsync("/BookFormats?Paging.PageSize=100");
    ...
    {
        var r1 = await (await _client.GetAsync("/BookFormats?Paging.PageSize=1")).Content
            .ReadFromJsonAsync<PageResult<ListBookFormatsQueryDto>>();
        var r2 = await (await _client.GetAsync("/BookFormats?Paging.PageSize=100")).Content
            .ReadFromJsonAsync<PageResult<ListBookFormatsQueryDto>>();
    ...
    {
        var response = await _client.GetAsync("/BookFormats?Paging.Page=2&Paging.PageSize=1");
